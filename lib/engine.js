  ...
  if (rateBTCUSD60.length > 0){
    ...
    for (let j = 0; j < bots.length; j++){
      ...
      if (isBuyNow && !passed){
        ...
        console.log(bot.name, 'buy now rule');
        // first buy
        ...
        console.log(bot.name, '1, 2 buy rules');
        ...
        if (incAmount > 0 && incMin > 0 && incBuyAmount > 10){
          console.log(bot.name, 'inc buy rule');
        ...
        if (decAmount > 0 && decMin > 0 && decBuyAmount > 10){
          console.log(bot.name, 'dec buy rule');
          // buy when decrease
...
const buy = async(bot, amount) => {
  ...
  if (result.success && result.balance > amount && amount > 10){
    console.log(bot.name, 'make buy order', botID);
    const buyParams = {
      type: 'market',
      product_id: 'BTC-USD',
      funds: amount
    ...
    console.log(bot.name, "can't buy insufficient balance");
    await Bot.findOneAndUpdate({_id: botID}, {$set: {state: 'Paused'}});
  }
}
