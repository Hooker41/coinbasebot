...
const checkBuyRules = async() => {
  if (rateBTCUSD60.length > 0){
    ...
    for (let j = 0; j < bots.length; j++){
      ...
      const incMin = buyRule.increaseMin;
      const incBuyAmount = buyRule.increaseBuyAmount;
      const decAmount = buyRule.decreaseAmount;
      const decMin = buyRule.decreaseMin;
      if ((incAmount <= 0 || incMin <= 0 || incBuyAmount <= 0) && (decAmount <= 0 || decMin <= 0)){
        await Bot.findOneAndUpdate({_id: bot._id}, {$set: {state: 'Paused'}});
        continue;
      }
      if (incAmount > 0 && incMin > 0 && incBuyAmount > 0){
        const currentMin = new Date().getMinutes();
        let minAgo = new Date().setMinutes(currentMin - parseInt(incMin), 0, 0);
      ...
      if (decAmount > 0 && decMin > 0){
        ...
        if (item && lastPrice > 0){
          ...
          console.log(changeForDec+' decreased in '+decMin+'min buy amount $25');
...
const buy = async(bot, amount) => {
  ...
  if (result.success && result.balance > amount){
...
const getBTCUSD = async() => {
  rateBTCUSD60 = await publicClient.getProductHistoricRates('BTC-USD', { granularity: 60 });
  setTimeout(() => {
    getBTCUSD();
  }, 4000);
}
